...
let c = fs.readFileSync('src/data/i18n_airdrops.ts', 'utf8');

const newTrReplacement = 	r: airdrops.map(a => {
  if (a.id === 'dow-protocol') {
    return {
      ...a,
      reward: 'Potansiyel Kazanç',
      category: 'Kategori',
      overview:  + JSON.stringify(fullTrOverview) + ,
      strategy:  + JSON.stringify(fullTrStrategy) + ,
      steps:  + JSON.stringify(fullTrSteps) + ,
      faqs:  + JSON.stringify(fullTrFaqs) + 
    };
  }
  return {
    ...a,
...
c = c.replace(/tr: airdrops\.map\([\s\S]*?\}\)\),/, newTrReplacement);
fs.writeFileSync('src/data/i18n_airdrops.ts', c, 'utf8');
